...
from school.models import User,StaffLeave
...
def admin_leave_view(request):
    return render(request,'school/admin_leave.html')
...
@login_required(login_url='teacherlogin')
@user_passes_test(is_teacher)
def teacher_apply_leave_view(request):
    return render(request,'school/teacher_apply_leave.html')



def teacher_apply_leave_save_view(request):
    if request.method == "POST":
       
        leave_date = request.POST.get('leave_date')
        leave_message = request.POST.get('leave_message')
        
        staff = User.objects.get(user = request.user.id)
        leave = StaffLeave(
            user = staff,
            data = leave_date,
        ...
        leave.save()
    return redirect('teacher-apply-leave')
...
@login_required(login_url='studentlogin')
@user_passes_test(is_student)
def student_apply_leave_view(request):
    return render(request,'school/student_apply_leave.html')
